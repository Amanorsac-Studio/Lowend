...
| pluginval ≥ 8, auval (B14) | OPEN | pluginval is not installed on this machine and was not downloaded without permission. `auval` is a step in `build-mac-pkg.sh` and fails the build if it does not pass; it has never run. |
